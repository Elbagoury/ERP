...
import erp
from erp.modules.db import FunctionStatus
from erp.osv.expression import get_unaccent_wrapper
from .. import SUPERUSER_ID
from erp.sql_db import TestCursor
from erp.tools import (config, existing_tables, lazy_classproperty,
                        lazy_property, sql, Collector, OrderedSet)
from erp.tools.func import locked
from erp.tools.lru import LRU

_logger = logging.getLogger(__name__)
_schema = logging.getLogger('erp.schema')
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                erp.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                erp.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        self._init = True
        self._assertion_report = erp.tests.runner.ERPTestResult()
        ...
        self._db = erp.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = erp.modules.db.has_unaccent(cr)
            self.has_trigram = erp.modules.db.has_trigram(cr)
