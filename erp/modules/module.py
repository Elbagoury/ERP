...
import erp
import erp.tools as tools
import erp.release as release
from erp.tools import pycompat
...
# ad_paths is a deprecated alias, please use erp.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"erp.modules.module.ad_paths" is a deprecated proxy to '
        '"erp.addons.__path__".', DeprecationWarning, stacklevel=2)
    return erp.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "erp.addons".',
    ...
    def load_module(self, name):
        ...
        erp_name = re.sub(r'^openerp.addons.(\w+)$', r'erp.addons.\g<1>', name)

        erp_module = sys.modules.get(erp_name)
        if not erp_module:
            erp_module = importlib.import_module(erp_name)

        sys.modules[name] = erp_module

        return erp_module

class ERPHook(object):
    """ Makes erp package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to erp.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'erp\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `erp.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^erp.addons.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("erp.addons.base.maintenance.migrations", "erp.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``erp.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook erp.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in erp.addons.__path__:
        erp.addons.__path__.append(dd)

    # hook erp.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in erp.addons.__path__:
            erp.addons.__path__.append(ad)

    # hook erp.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in erp.addons.__path__ and os.path.isdir(base_path):
        erp.addons.__path__.append(base_path)

    # hook erp.upgrade on upgrade-path
    from erp import upgrade
    ...
    # create decrecated module alias from erp.addons.base.maintenance.migrations to erp.upgrade
    spec = importlib.machinery.ModuleSpec("erp.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["erp.addons.base.maintenance"] = maintenance_pkg
    sys.modules["erp.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to erp and "crm"-like to erp.addons
    if not getattr(initialize_sys_path, 'called', False): # only initialize once
        sys.meta_path.insert(0, UpgradeHook())
        sys.meta_path.insert(0, ERPHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in erp.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = erp.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in erp.addons.__path__:
...
def load_openerp_module(module_name):
    ...
    try:
        __import__('erp.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['erp.addons.' + module_name], info['post_load'])()
