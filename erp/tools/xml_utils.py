...
class erp_resolver(etree.Resolver):
    """ERP specific file resolver that can be added to the XML Parser.
...
def _check_with_xsd(tree_or_str, stream, env=None, prefix=None):
    ...
    :param erp.api.Environment env: If it is given, it enables resolving the
    ...
    if env:
        parser.resolvers.add(erp_resolver(env, prefix))
...
def load_xsd_files_from_url(env, url, file_name, force_reload=False,
    ...
    :param erp.api.Environment env: environment of calling module
    ...
    :rtype: erp.api.ir.attachment | bool
...
def validate_xml_from_attachment(env, xml_content, xsd_name, reload_files_function=None, prefix=None):
    ...
    :return: the result of the function :func:`erp.tools.xml_utils._check_with_xsd`
    """
