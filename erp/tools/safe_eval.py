...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False, filename=None):
    ...
    except erp.exceptions.UserError:
        raise
    except erp.exceptions.RedirectWarning:
        raise
...
`erp.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `erp.tools.safe_eval`.
""")
    return d
