...
from erp.tools.misc import file_open
...
def add_banner(pdf_stream, text=None, logo=False, thickness=2 * cm):
    """ Add a banner on a PDF in the upper right corner, with ERP's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display ERP's logo in the banner.
    ...
    erp_logo = Image.open(file_open('base/static/img/main_partner-image.png', mode='rb'))
    erp_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        logo and can.drawImage(
            ImageReader(erp_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
class ERPPdfFileReader(PdfFileReader):
...
class ERPPdfFileWriter(PdfFileWriter):
    ...
    def embed_erp_attachment(self, attachment, subtype=None):
        assert attachment, "embed_erp_attachment cannot be called without attachment."
