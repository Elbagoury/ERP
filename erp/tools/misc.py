...
from erp.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if erp.tools.config['db_host']:
        env['PGHOST'] = erp.tools.config['db_host']
    if erp.tools.config['db_port']:
        env['PGPORT'] = str(erp.tools.config['db_port'])
    if erp.tools.config['db_user']:
        env['PGUSER'] = erp.tools.config['db_user']
    if erp.tools.config['db_password']:
        env['PGPASSWORD'] = erp.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',)):
    ...
    addons_paths = erp.addons.__path__ + [root_path]
...
def file_open(name, mode="r", filter_ext=None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/erp/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = erp.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('erp.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('erp.foo.bar'):
            do_suff()
    """
...
def ignore(*exc):
    warnings.warn("Since 16.0 `erp.tools.ignore` is replaced by `contextlib.suppress`.", DeprecationWarning, stacklevel=2)
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = erp.fields.Datetime.from_string(value)
            value = erp.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = erp.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = erp.fields.Datetime.from_string(value)
    else:
        timestamp = value
