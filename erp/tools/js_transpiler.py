"""
This code is what let us use ES6-style modules in erp.
Classic ERP modules are composed of a top-level :samp:`erp.define({name},{body_function})` call.
This processor will take files starting with an `@erp-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** erp-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual erp one with minimal changes.
...
def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom erp modules.
    ...
    legacy_erp_define = get_aliased_erp_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_erp_define, module_path),
    ]
    for s in steps:
        content = s(content)
    if legacy_erp_define:
        content += legacy_erp_define
    return content
...
def url_to_module_path(url):
    """
    ERP modules each have a name. (erp.define("<the name>", async function (require) {...});
...
def wrap_with_erp_define(module_path, content):
    """
    Wraps the current content (source code) with the erp.define call.
    Should logically be called once all other operations have been performed.
    """
    return f"""erp.define({module_path!r}, async function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
def is_erp_module(content):
    """
    Detect if the file is a native erp module.
    We look for a comment containing @erp-module.

    :param content: source code
    :return: is this a erp module that need transpilation ?
    """
    result = ODOO_MODULE_RE.match(content)
    return bool(result)


def get_aliased_erp_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the erp source base being:
    ...
    > erp.define("web/chrome/abstract_action", async function(require) {
    ...
    .. code-block:: javascript
        ...
        /** @erp-module */
        // after
        /** @erp-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @erp-module */
        // after
        /** @erp-module alias=web.AbstractAction default=false */
    ...
    if matchobj:
        ...
        if alias:
            if matchobj['default']:
                return """\nerp.define(`%s`, async function(require) {
                        return require('%s');
                        });\n""" % (alias, module_path)
            else:
                return """\nerp.define(`%s`, async function(require) {
