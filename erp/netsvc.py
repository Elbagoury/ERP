...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('erp.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='erp')
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='erp.tools.image')
...
DEFAULT_LOG_CONFIGURATION = [
    'erp.http.rpc.request:INFO',
    'erp.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['erp:DEBUG', 'erp.sql_db:INFO', 'erp.http.rpc:DEBUG'],
    'debug_rpc': ['erp:DEBUG', 'erp.sql_db:INFO', 'erp.http.rpc.request:DEBUG'],
    'debug': ['erp:DEBUG', 'erp.sql_db:INFO'],
    'debug_sql': ['erp.sql_db:DEBUG'],
    'info': [],
    'runbot': ['erp:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['erp:WARNING', 'werkzeug:WARNING'],
    'error': ['erp:ERROR', 'werkzeug:ERROR'],
    'critical': ['erp:CRITICAL', 'werkzeug:CRITICAL'],
}
