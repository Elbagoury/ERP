...
(function (exports) {
    ...
    const BLACKLISTED_MENUS = [
        ...
        "payment_erp.menu_adyen_account",
        "payment_erp.root_adyen_menu",
    ];

    const { isEnterprise } = erp.info;
    const { onWillStart } = owl;
    let appsMenusOnly = false;
    const isStudioInstalled = "@web_studio/studio_service" in erp.__DEBUG__.services;
    let actionCount = 0;
    ...
    function ensureSetup() {
        ...
        setupDone = true;
        const env = erp.__WOWL_DEBUG__.root.env;
        env.bus.addEventListener("ACTION_MANAGER:UI-UPDATED", () => {
            actionCount++;
        });

        const AbstractController = erp.__DEBUG__.services["web.AbstractController"];
        ...
        const { patch } = erp.__DEBUG__.services["@web/core/utils/patch"];
        const { WithSearch } = erp.__DEBUG__.services["@web/search/with_search/with_search"];
        ...
        // This test file is not respecting ERP module dependencies.
        // The following module might not be loaded (eg. if mail is not installed).
        const DiscussWidgetModule = erp.__DEBUG__.services["@mail/widgets/discuss/discuss"];
    ...
    async function _clickEverywhere(xmlId) {
        ...
        console.log(`ERP flavor: ${isEnterprise ? "Enterprise" : "Community"}`);
