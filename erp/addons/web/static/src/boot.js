...
(function () {
    ...
    erp.define = function () {
        ...
        var name = typeof args[0] === "string" ? args.shift() : "__erp_job" + jobUID++;
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: erp.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name,
                };
                if (jobs[k].error) {
                ...
                var deps = erp.__DEBUG__.getDependencies(job.name);
            ...
            missing = erp.__DEBUG__.getMissingJobs();
            failed = erp.__DEBUG__.getFailedJobs();
     ...
     */
    erp.ready = async function (serviceName) {
        function match(name) {
    ...
    erp.runtimeImport = function (moduleName) {
