erp.define('web.test_utils_mock', function (require) {
"use strict";
...
const { Component } = require("@erp/owl");
 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any ERP widget)
...
async function addMockEnvironmentOwl(Component, params, mockServer) {
    ...
    function cleanUp() {
        ...
        $('.blockUI').remove(); // fixme: move to qunit_config in ERPAfterTestHook?
 ...
 *   used to generate mock answers for every kind of routes supported by erp
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught ERP
 *   events.
...
async function addMockEnvironment(widget, params) {
    ...
    // make sure all other ERP events bubbling up are intercepted
