...
from erp import http
from erp.http import content_disposition, dispatch_rpc, request
from erp.service import db
from erp.tools.misc import file_open, str2bool
from erp.tools.translate import _

from erp.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = erp.tools.config.verify_admin_password('admin')
        d['list_db'] = erp.tools.config['list_db']
        d['langs'] = erp.service.db.exp_list_lang()
        d['countries'] = erp.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = erp.service.db.list_db_incompatible(d['databases'])
        except erp.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = erp.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format='zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            erp.service.db.check_super(master_pwd)
            ...
            dump_stream = erp.service.db.dump_db(name, None, backup_format)
