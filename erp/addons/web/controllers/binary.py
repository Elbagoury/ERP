...
try:
    ...
    from erp.tools._vendor.send_file import send_file

import erp
import erp.modules.registry
from erp import http, _
from erp.exceptions import AccessError, UserError
from erp.http import request
from erp.modules import get_resource_path
from erp.tools import file_open, file_path, replace_exceptions
from erp.tools.mimetypes import guess_mimetype
from erp.tools.image import image_guess_size_from_field_name


_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
ERP is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however ERP is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if erp.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=erp.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        dbname = request.db
        uid = (request.session.uid if dbname else None) or erp.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                registry = erp.modules.registry.Registry(dbname)
