...
from erp.http import content_disposition, request
from erp.tools import lazy_property, osutil, pycompat
from erp.tools.misc import xlsxwriter
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name='',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: erp.tools.ustr(field[1].get('string', '').lower()))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from ERP's export data to whatever the
