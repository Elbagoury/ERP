...
const LinkPopoverWidget = Widget.extend({
    ...
    start() {
        ...
        .on('show.bs.popover.link_popover', () => {
            this.options.wysiwyg.erpEditor.observerUnactive('show.bs.popover');
            this._loadAsyncLinkPreview();
            popoverShown = true;
        })
        .on('inserted.bs.popover', () => {
            this.options.wysiwyg.erpEditor.observerActive('show.bs.popover');
        })
        .on('hide.bs.popover.link_popover', () => {
            this.options.wysiwyg.erpEditor.observerUnactive('hide.bs.popover');
            popoverShown = false;
        })
        .on('hidden.bs.popover.link_popover', () => {
            this.options.wysiwyg.erpEditor.observerActive('hide.bs.popover');
        ...
        if (document !== this.options.wysiwyg.erpEditor.document) {
            $(this.options.wysiwyg.erpEditor.document).on('mouseup.link_popover', onClickDocument);
        }
    ...
    destroy() {
        ...
        $(this.options.wysiwyg.erpEditor.document).off('.link_popover');
...
LinkPopoverWidget.createFor = async function (parent, targetEl, options) {
    ...
    if (wysiwyg) {
        wysiwyg.erpEditor.observerUnactive('LinkPopoverWidget');
    }
    await popoverWidget.appendTo(targetEl)
    if (wysiwyg) {
        wysiwyg.erpEditor.observerActive('LinkPopoverWidget');
    }
