erp.define('web_editor.wysiwyg', function (require) {
'use strict';
...
const ERPEditorLib = require('@web_editor/js/editor/erp-editor/src/ERPEditor');
...
const ERPEditor = ERPEditorLib.ERPEditor;
const getDeepRange = ERPEditorLib.getDeepRange;
const getInSelection = ERPEditorLib.getInSelection;
const isBlock = ERPEditorLib.isBlock;
const rgbToHex = ERPEditorLib.rgbToHex;
const preserveCursor = ERPEditorLib.preserveCursor;
const closestElement = ERPEditorLib.closestElement;
const setSelection = ERPEditorLib.setSelection;
const endPos = ERPEditorLib.endPos;
const hasValidSelection = ERPEditorLib.hasValidSelection;
const parseHTML = ERPEditorLib.parseHTML;
...
const Wysiwyg = Widget.extend({
    ...
    startEdition: async function () {
        ...
        this.erpEditor = new ERPEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.erpEditor.addEventListener('contentChanged', function () {
        ...
        if (this.erpEditor.document !== document) {
            this.erpEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.erpEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.erpEditor.document.addEventListener("keyup", this._signalOnline, true);
        }

        this._initialValue = this.getValue();
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $wrapwrap[0].addEventListener('scroll', this.erpEditor.multiselectionRefresh, { passive: true });
        ...
        this._observeERPFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.erpEditor.observerUnactive();
                const $field = $(this);
        ...
        this.$editable.on('dblclick', mediaSelector, function () {
            if (this.isContentEditable || (this.parentElement && this.parentElement.isContentEditable)) {
                ...
                const selection = self.erpEditor.document.getSelection();
        ...
        if (options.snippets) {
            $(this.erpEditor.document.body).addClass('editor_enable');
        ...
        $(this.erpEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.erpEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.erpEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.erpEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        this.setCSSVariables(this.snippetsMenu ? this.snippetsMenu.el : this.toolbar.el);

        this.erpEditor.addEventListener('preObserverActive', () => {
            ...
            this._setONotEditable(this.erpEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.erpEditor.isMobile) {
                $(this.erpEditor.editable).before(this.toolbar.$el);
            } else {
    ...
    setupCollaboration(collaborationChannel) {
        ...
        this._getNewPtp = () => {
            ...
            return new PeerToPeer({
                ...
                onRequest: {
                    ...
                    get_missing_steps: (params) => this.erpEditor.historyGetMissingSteps(params.requestPayload),
                    get_history_from_snapshot: () => this.erpEditor.historyGetSnapshotSteps(),
                    get_collaborative_selection: () => this.erpEditor.getCurrentCollaborativeSelection(),
                },
                onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                    switch (notificationName) {
                        case 'ptp_remove':
                            this.erpEditor.multiselectionRemove(notificationPayload);
                            break;
                        ...
                        case 'ptp_disconnect':
                            ...
                            this.erpEditor.multiselectionRemove(fromClientId);
                            break;
                        case 'rtc_data_channel_open': {
                            ...
                            if (!historySyncAtLeastOnce) {
                                ...
                                if (remoteSelection) {
                                    this.erpEditor.onExternalMultiselectionUpdate(remoteSelection);
                                }
                            }
                            break;
                        }
                        case 'oe_history_step':
                            ...
                            if (historySyncFinished) {
                                this.erpEditor.onExternalHistorySteps([notificationPayload]);
                            }
                            break;
                        case 'oe_history_set_selection': {
                            ...
                            this.erpEditor.onExternalMultiselectionUpdate(selection);
                            break;
                        }
                    }
                }
        ...
        const editorCollaborationOptions = {
            ...
            onHistoryMissingParentSteps: async ({ step, fromStepId }) => {
                ...
                this.ptp && this.erpEditor.onExternalHistorySteps(missingSteps.concat([step]));
            },
            onHistoryNeedSync: async () => {
                ...
                if (firstClientId !== this._currentClientId) {
                    ...
                    this.erpEditor.historyResetFromSteps(historySteps);
    ...
    destroy: function () {
        ...
        if (this.erpEditor) {
            this.erpEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.erpEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.erpEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.erpEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            for (const observer of this._oNotEditableObservers.values()) {
                observer.disconnect();
            }
            this.erpEditor.destroy();
        }
        ...
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $('#wrapwrap')[0].removeEventListener('scroll', this.erpEditor.multiselectionRefresh, { passive: true });
        }
        $(this.$root).off('click');
        if (this.linkPopover) {
    ...
    renderElement: function () {
        ...
        if (this.options.resizable && !device.isMobile) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper erp-editor">');
            this.$root = $wrapper;
    ...
    getValue: function (options) {
        ...
        this.erpEditor && this.erpEditor.cleanForSave($editable[0]);
    ...
    cancel: function (reload) {
        ...
        return new Promise((resolve, reject) => {
            if (!this.erpEditor.historySize().length) {
                resolve();
            } else {
    ...
    setValue: function (value) {
        this.erpEditor.resetContent(value);
    },
    /**
     * Undo one step of change in the editor.
     */
    undo: function () {
        this.erpEditor.historyUndo();
    },
    /**
     * Redo one step of change in the editor.
     */
    redo: function () {
        this.erpEditor.historyRedo();
    },
    /**
     ...
     */
    focus: function () {
        if (this.erpEditor && !this.erpEditor.historyResetLatestComputedSelection(true)) {
            ...
            const selection = this.erpEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    },
    getDeepRange() {
        return getDeepRange(this.erpEditor.editable);
    },
    closestElement(...args) {
        return closestElement(...args);
    },
    async cleanForSave() {
        this.erpEditor && this.erpEditor.cleanForSave();
    ...
    isSelectionInEditable: function () {
        return this.erpEditor.isSelectionInEditable();
    },
    /**
     * Start or resume the ERP field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeERPFieldChanges: function () {
        ...
        if (this.erpFieldObservers) {
            for (let observerData of this.erpFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const erpFieldSelector = '[data-oe-model], [data-oe-translation-initial-sha]';
            const $erpFields = this.$editable.find(erpFieldSelector);
            this.erpFieldObservers = [];

            $erpFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.erpEditor.filterMutationRecords(mutations);
                    if (!mutations.length) {
                        return;
                    }
                    let $node = $(field);
                    let $nodes = $erpFields.filter(function () {
                        return this !== field;
                    });
                ...
                this.erpFieldObservers.push({field: field, observer: observer});
            });
        }
    },
    /**
     ...
     */
    _pauseERPFieldObservers: function () {
        for (let observerData of this.erpFieldObservers) {
    ...
    openLinkToolsFromSelection() {
        const targetEl = this.erpEditor.document.getSelection().getRangeAt(0).startContainer;
    ...
    toggleLinkTools(options = {}) {
        const linkEl = getInSelection(this.erpEditor.document, 'a');
        ...
        if (this.snippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.linkTools) {
                ...
                if (!this.linkTools || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkTools.$link[0])) {
                    const { link } = Link.getOrCreateLink({
                        containerNode: this.erpEditor.editable,
                    ...
                    this.linkTools = new weWidgets.LinkTools(this, {wysiwyg: this, noFocusUrl: options.noFocusUrl}, this.erpEditor.editable, linkToolsData, $btn, link );
                }
                ...
                const _onClick = ev => {
                    if (
                        ...
                        this.erpEditor.document.removeEventListener('click', _onClick, true);
                        document.removeEventListener('click', _onClick, true);
                    }
                };
                this.erpEditor.document.addEventListener('click', _onClick, true);
            ...
            const restoreSelection = preserveCursor(this.erpEditor.document);
            linkDialog.open();
            linkDialog.on('save', this, data => {
                ...
                this.erpEditor.historyStep();
                link = linkWidget.$link[0];
                this.erpEditor.setContenteditableLink(linkWidget.$link[0]);
    ...
    removeLink() {
        if (this.snippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            ...
            this.erpEditor.execCommand('unlink');
        }
    },
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.erpEditor.document.getSelection();
        ...
        const $editable = $(ERPEditorLib.closestElement(range.startContainer, '.o_editable') || this.erpEditor.editable);
    ...
    _onMediaDialogSave: function (params, element) {
        ...
        if (params.node) {
            ...
            this.erpEditor.unbreakableStepUnactive();
            this.erpEditor.historyStep();
        } else {
            return this.erpEditor.execCommand('insert', element);
        }
    ...
    getInSelection(selector) {
        return getInSelection(this.erpEditor.document, selector);
    },
    /**
    ...
    _configureToolbar: function (options) {
        ...
        $toolbar.find('#image-crop').click(e => {
            ...
            new weWidgets.ImageCropWidget(this, this.lastMediaClicked).appendTo(this.erpEditor.document.body);
            this.erpEditor.toolbarHide();
            $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.erpEditor.toolbarShow());
        });
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.erpEditor.document});
            const mouseup = () => {
                $('#image-transform').toggleClass('active', $image.is('[style*="transform"]'));
            };
            $(this.erpEditor.document).on('mouseup', mouseup);
            const mousedown = mousedownEvent => {
                if (!$(mousedownEvent.target).closest('.transfo-container').length) {
                    $image.transfo('destroy');
                    $(this.erpEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                }
            ...
            $(this.erpEditor.document).on('mousedown', mousedown);
        });
        ...
        setTimeout(() => {
            ...
            if (!options.snippets && scrollableContainer.length) {
                this.erpEditor.addDomListener(
                    scrollableContainer[0],
                    'scroll',
                    this.erpEditor.updateToolbarPosition.bind(this.erpEditor),
                );
            }
        }, 0);
    },
        ...
        for (const elem of $dropdownContent) {
            ...
            $dropdown.on('show.bs.dropdown', () => {
                ...
                mutex.exec(() => {
                    ...
                    this.erpEditor.historyStash();
                    colorpicker = new ColorPaletteWidget(this, {
                        excluded: ['transparent_grayscale'],
                        $editable: $(this.erpEditor.editable), // Our parent is the root widget, we can't retrieve the editable section from it...
                    ...
                    colorpicker.on('custom_color_picked color_picked', null, ev => {
                        if (hadNonCollapsedSelection) {
                            this.erpEditor.historyResetLatestComputedSelection(true);
                        }
                        // Unstash the mutations now that the color is picked.
                        this.erpEditor.historyUnstash();
                        ...
                        // hides it).
                        if (this.erpEditor.deselectTable() && hasValidSelection(this.erpEditor.editable)) {
                            this.erpEditor.document.getSelection().collapseToStart();
                        }
                    ...
                    colorpicker.on('color_hover', null, ev => {
                        ...
                        this.erpEditor.historyPauseSteps();
                        try {
                            this._processAndApplyColor(eventName, ev.data.color, true);
                        } finally {
                            this.erpEditor.historyUnpauseSteps();
                        }
                    });
                    colorpicker.on('color_leave', null, ev => {
                        this.erpEditor.historyRevertCurrentStep();
                    });
    ...
    _processAndApplyColor: function (eventName, color, previewMode) {
        ...
        const coloredElements = this.erpEditor.execCommand('applyColor', color, eventName === 'foreColor' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        ...
        this.erpEditor.updateColorpickerLabels({
    ...
    _updateEditorUI: function (e) {
        let selection = this.erpEditor.document.getSelection();
        const anchorNode = selection.anchorNode;
        if (anchorNode && closestElement(anchorNode, '[data-oe-protected="true"]')) {
            return;
        }

        this.erpEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.erpEditor.document.defaultView;
        ...
        selection = this.erpEditor.document.getSelection();
        ...
        if (isInMedia) {
            ...
            const selection = this.erpEditor.document.getSelection();
            const range = this.erpEditor.document.createRange();
        ...
        const link = getInSelection(this.erpEditor.document, this.customizableLinksSelector);
        if (isInMedia || (link && link.isContentEditable)) {
            ...
            this.tooltipTimeouts.push(setTimeout(() => {
                ...
                this.erpEditor.observerUnactive();
    ...
    _updateMediaJustifyButton: function (commandState) {
        ...
        // Change the ID to prevent ERPEditor from controlling it as this is
        ...
        const commands = [
            {
                ...
                isDisabled: () => !this.erpEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.erpEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.erpEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.erpEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (res && res[0] && res[0].signature) {
                        this.erpEditor.execCommand('insert', parseHTML(res[0].signature));
                    }
                },
            },
            {
                ...
                callback: () => this.erpEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                isDisabled: () => {
                    if (!this.erpEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
                    const anchor = this.erpEditor.document.getSelection().anchorNode;
            ...
            },
            {
                ...
                fontawesome: 'fa-columns',
                callback: () => this.erpEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                isDisabled: () => {
                    if (!this.erpEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
            ...
            },
            {
                ...
                fontawesome: 'fa-columns',
                callback: () => this.erpEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                isDisabled: () => {
                    if (!this.erpEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
            ...
            },
            {
                ...
                fontawesome: 'fa-columns',
                callback: () => this.erpEditor.execCommand('columnize', 0),
                isDisabled: () => {
                    if (!this.erpEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
    ...
    _saveElement: function ($el, context, withLang) {
        ...
        // remove ZeroWidthSpace from erp field value
        // ZeroWidthSpace may be present from ERPEditor edition process
    ...
    _onSelectionChange() {
        if (this.options.autohideToolbar) {
            ...
            if (isVisible && !this.erpEditor.document.getSelection().isCollapsed) {
                this.linkPopover.hide();
            }
        }
    },
    ...
    _signalOnline: async function () {
        ...
        try {
            ...
            if (currentContent !== dbContent && dbRecordDate !== currentRecordDate) {
                ...
                $dialogContent.append($(this.erpEditor.editable).clone());
    ...
    resetEditor: function (value, options) {
        ...
        this._startCollaborationTime = new Date().getTime();
        this.ptp = this._getNewPtp();
        this.erpEditor.collaborationSetClientId(this._currentClientId);
        this.setValue(value);
        this.erpEditor.historyReset();
        // Wait until editor is focused to join the peer to peer network.
        this.$editable[0].addEventListener('focus', this._joinPeerToPeer);
                    ...
                    for (const record of records) {
                        if (record.type === 'attributes' && record.attributeName === 'class') {
                            ...
                            if (!node.classList.contains('o_not_editable')) {
                                this.erpEditor.observerUnactive('_setONotEditable');
                                node.removeAttribute('contenteditable');
                                this.erpEditor.observerActive('_setONotEditable');
                                observer.disconnect();
...
erp.define('web_editor.widget', function (require) {
'use strict';
    return {
