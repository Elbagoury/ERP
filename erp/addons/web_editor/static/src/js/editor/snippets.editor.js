erp.define('web_editor.snippet.editor', function (require) {
'use strict';
...
var SnippetEditor = Widget.extend({
    ...
    removeSnippet: async function (shouldRecordUndo = true) {
        this.options.wysiwyg.erpEditor.unbreakableStepUnactive();
        ...
        if (shouldRecordUndo) {
            this.options.wysiwyg.erpEditor.historyStep();
        }
    },
    /**
    ...
    clone: async function (recordUndo) {
        ...
        if (recordUndo) {
            this.options.wysiwyg.erpEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.trigger_up('call_for_each_child_snippet', {
                $snippet: $clone,
    ...
    _onDragAndDropStart: function () {
        ...
        this.options.wysiwyg.erpEditor.automaticStepUnactive();
        var self = this;
        this.dragState = {};
    ...
    _onDragAndDropStop: function (ev, ui) {
        this.options.wysiwyg.erpEditor.automaticStepActive();
        this.options.wysiwyg.erpEditor.automaticStepSkipStack();
        this.options.wysiwyg.erpEditor.unbreakableStepUnactive();
        ...
        this.options.wysiwyg.erpEditor.observerActive('dragAndDropMoveSnippet');
        if (this.dropped) {
            if (prev) {
...
var SnippetsMenu = Widget.extend({
    ...
    async start() {
        ...
        $(this.options.wysiwyg.erpEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.erpEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.erpEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            ...
            if ($undoButton.length) {
                const updateHistoryButtons = () => {
                    $undoButton.attr('disabled', !this.options.wysiwyg.erpEditor.historyCanUndo());
                    $redoButton.attr('disabled', !this.options.wysiwyg.erpEditor.historyCanRedo());
                };
                this.options.wysiwyg.erpEditor.addEventListener('historyStep', updateHistoryButtons);
                this.options.wysiwyg.erpEditor.addEventListener('observerApply', () => {
                    $(this.options.wysiwyg.erpEditor.editable).trigger('content_changed');
                });
                this.options.wysiwyg.erpEditor.addEventListener('historyRevert', _.debounce(() => {
        ...
        const smoothScrollOptions = this._getScrollOptions({
            jQueryDraggableOptions: {
                ...
                start: function () {
                    ...
                    const doc = self.options.wysiwyg.erpEditor.document;
                    $(doc.body).addClass('oe_dropzone_active');

                    self.options.wysiwyg.erpEditor.automaticStepUnactive();

                    self.$el.find('.oe_snippet_thumbnail').addClass('o_we_already_dragging');
                    self.options.wysiwyg.erpEditor.observerUnactive('dragAndDropCreateSnippet');
                ...
                stop: async function (ev, ui) {
                    ...
                    self.options.wysiwyg.erpEditor.automaticStepUnactive();
                    self.options.wysiwyg.erpEditor.automaticStepSkipStack();
                    $toInsert.removeClass('oe_snippet_body');
                    ...
                    self.options.wysiwyg.erpEditor.observerActive('dragAndDropCreateSnippet');
                    ...
                    if (dropped) {
                        ...
                        _.defer(async function () {
                            ...
                            self.options.wysiwyg.erpEditor.unbreakableStepUnactive();
                            self.options.wysiwyg.erpEditor.historyStep();
   ...
   _onSnippetDragAndDropStart: function () {
    ...
    _onSnippetClick() {
        ...
        for (const el of $els) {
            if (!el.children.length) {
                $(el).erpBounce('o_we_snippet_area_animation');
            }
        }
    },
    /**
    ...
    _addToolbar(toolbarMode = "text") {
        ...
        this.options.wysiwyg.erpEditor.bindExecCommand($customizeTableBlock[0]);
    ...
    _checkEditorToolbarVisibility: function (e) {
        ...
        const selection = this.options.wysiwyg.erpEditor.document.getSelection();
    ...
    _onRequestEditable: function (ev) {
        ev.data.callback($(this.options.wysiwyg.erpEditor.editable));
    },
    /**
