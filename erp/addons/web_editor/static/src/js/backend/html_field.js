...
import {
    ...
    getRangePosition
} from '@web_editor/js/editor/erp-editor/src/utils/utils';
...
export class HtmlField extends Component {
    ...
    _filterPowerBoxCommands(commands) {
        ...
        if (this.wysiwyg && this.wysiwyg.erpEditor) {
            const selection = this.wysiwyg.erpEditor.document.getSelection();
    ...
    async startWysiwyg(wysiwyg) {
        ...
        this.wysiwyg.erpEditor.addEventListener("historyStep", () =>
    ...
    toggleCodeView() {
        ...
        this.wysiwyg.erpEditor.observerUnactive('toggleCodeView');
        if (this.state.showCodeView) {
            this.wysiwyg.erpEditor.toolbarHide();
            const value = this.wysiwyg.getValue();
            this.props.update(value);
        } else {
            this.wysiwyg.erpEditor.observerActive('toggleCodeView');
    ...
    onDynamicPlaceholderValidate(chain, defaultValue) {
        if (chain) {
            ...
            this.wysiwyg.erpEditor.execCommand('insert', t);
        }
    }
    ...
    async commitChanges({ urgent } = {}) {
        if (this._isDirty() || urgent) {
            ...
            if (this.wysiwyg) {
                ...
                this.wysiwyg.erpEditor.observerUnactive('commitChanges');
                await this.wysiwyg.saveModifiedImages();
                if (this.props.isInlineStyle) {
                    await this._toInline();
                }
                this.wysiwyg.erpEditor.observerActive('commitChanges');
            }
    ...
    async _toInline() {
        ...
        const $erpEditor = $editable.closest('.erp-editor-editable');
        ...
        $erpEditor.removeClass('erp-editor-editable');
        $editable.html(html);

        await toInline($editable, this.cssRules, this.wysiwyg.$iframe);
        $erpEditor.addClass('erp-editor-editable');
...
HtmlField.extractProps = ({ attrs, field }) => {
    ...
    return {
        ...
        codeview: Boolean(erp.debug && attrs.options.codeview),
        placeholder: attrs.placeholder,
