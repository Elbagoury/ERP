/** @erp-module alias=web_editor.field.html */
'use strict';
...
import {
    ...
    rightPos,
    getRangePosition
} from '../editor/erp-editor/src/utils/utils';
...
var FieldHtml = basic_fields.DebouncedField.extend(DynamicPlaceholderFieldMixin).extend(TranslatableFieldMixin, {
    ...
    openDynamicPlaceholder: async function (baseModel, chain = []) {
        ...
        const onFieldChanged = (ev) => {
            this.wysiwyg.erpEditor.editable.focus();
    ...
    _onLoadWysiwyg: function () {
        ...
        if (this.nodeOptions.cssEdit && this.wysiwyg) {
            ...
            $button.css({
                ...
                position: 'absolute',
                top: '5px',
                [_t.database.parameters.direction === 'rtl' ? 'left' : 'right']: erp.debug && this.nodeOptions.codeview ? '40px' : '5px',
            });
        }
        $container.append($button);
        if (erp.debug && this.nodeOptions.codeview) {
