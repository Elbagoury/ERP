...
from erp.addons.payment.controllers.portal import PaymentPortal
from erp.addons.payment.tests.http_common import PaymentHttpCommon
...
class TestFlows(PaymentHttpCommon):
    ...
    def _test_flow(self, flow):
        ...
        with mute_logger('erp.addons.payment.models.payment_transaction'):
    ...
    def test_transaction_wrong_flow(self):
        ...
        with mute_logger('erp.http'):
            response = self._portal_transaction(**transaction_values)
        self.assertIn(
            "erp.exceptions.UserError: The payment should either be direct, with redirection, or made by a token.",
            response.text)
    ...
    def test_transaction_wrong_token(self):
        ...
        self.assertIn(
            "erp.exceptions.ValidationError: The access token is invalid.",
            response.text)
    ...
    @mute_logger('erp.addons.payment.models.payment_transaction')
    def test_direct_payment_triggers_no_payment_request(self):
        self.authenticate(self.portal_user.login, self.portal_user.login)
        self.partner = self.portal_partner
        self.user = self.portal_user
        with patch(
            'erp.addons.payment.models.payment_transaction.PaymentTransaction'
