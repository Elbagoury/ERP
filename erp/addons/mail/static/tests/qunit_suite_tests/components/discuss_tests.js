...
QUnit.test('receive new chat message: out of erp focus (notification, channel)', async function (assert) {
    assert.expect(4);
    ...
    const { env, openDiscuss } = await start({
        services: {
            presence: makeFakePresenceService({ isERPFocused: () => false }),
        },
    });
    await openDiscuss();
    ...
    // simulate receiving a new message with erp focused
    await afterNextRender(() => {
...
QUnit.test('receive new chat message: out of erp focus (notification, chat)', async function (assert) {
    assert.expect(4);
...
QUnit.test('receive new chat messages: out of erp focus (tab title)', async function (assert) {
    assert.expect(12);
    ...
    const mailChannel1 = pyEnv['mail.channel'].searchRead([['id', '=', mailChannelId1]])[0];
    // simulate receiving a new message in chat 1 with erp focused
    await afterNextRender(() => {
        pyEnv['bus.bus']._sendone(mailChannel1, 'mail.channel/new_message', {
            'id': mailChannelId1,
            'message': {
    ...
    // simulate receiving a new message in chat 2 with erp focused
    await afterNextRender(() => {
    ...
    // simulate receiving another new message in chat 2 with erp focused
    await afterNextRender(() => {
        pyEnv['bus.bus']._sendone(mailChannel2, 'mail.channel/new_message', {
            'id': mailChannelId2,
            'message': {
