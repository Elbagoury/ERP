...
import erp
from erp.http import Controller, route, dispatch_rpc, request
from erp.fields import Date, Datetime, Command
from erp.tools import lazy, ustr
from erp.tools.misc import frozendict
...
# User code must use the exceptions defined in ``erp.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, erp.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, erp.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, erp.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, erp.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, erp.exceptions.MissingError):
    ...
    #InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(erp.tools.exception_to_unicode(e), formatted_info)

    return xmlrpc.client.dumps(fault, allow_none=None, encoding=None)


class ERPMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, erp is sending binary as base64 string.
...
xmlrpc.client.Marshaller = ERPMarshaller
