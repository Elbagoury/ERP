...
from erp.exceptions import AccessDenied, AccessError
from erp.http import _request_stack

import erp.tools
from erp.tests import common
from erp.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, erp.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
