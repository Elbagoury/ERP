...
from erp import api, fields, models, tools, SUPERUSER_ID, _, Command
from erp.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from erp.exceptions import AccessDenied, AccessError, UserError, ValidationError
from erp.http import request, DEFAULT_LANG
from erp.osv import expression
from erp.service.db import check_super
from erp.tools import is_html_empty, partition, collections, frozendict, lazy_property
...
class Users(models.Model):
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: erp.exceptions.AccessDenied when old password is wrong
        :raise: erp.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this ERP portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your ERP is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "ERP in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.erp.com/documentation/16.0/administration/install/deploy.html#https for details.",
                    source
                )
