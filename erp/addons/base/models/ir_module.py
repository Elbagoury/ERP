...
import erp
from erp import api, fields, models, modules, tools, _
from erp.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from erp.exceptions import AccessDenied, UserError
from erp.osv import expression
from erp.tools.parse_version import parse_version
from erp.tools.misc import topological_sort
from erp.tools.translate import TranslationImporter
from erp.http import request
from erp.modules import get_module_path, get_module_resource
...
class Module(models.Model):
    ...
    license = fields.Selection([
        ...
        ('OPL-1', 'ERP Proprietary License v1.0'),
    ...
    def _button_immediate_function(self, function):
        ...
        try:
            ...
            raise UserError(_("ERP is currently processing a scheduled action.\n"
    ...
    def button_uninstall(self):
        un_installable_modules = set(erp.conf.server_wide_modules) & set(self.mapped('name'))
    ...
    def install_from_urls(self, urls):
        ...
        OPENERP = erp.release.product_name.lower()
        ...
        try:
            ...
            if urls.get(OPENERP):
                ...
                # copy all modules in the SERVER/erp/addons directory to the new "erp" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, OPENERP, 'addons', d)    # XXX 'erp' subdirectory ?
                ...
                _logger.info('Copy downloaded module `erp` to `%s`', server_dir)
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.erp.com/apps')
