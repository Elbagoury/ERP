erp.define("report.client_action", function (require) {
    "use strict";
    ...
    var AUTHORIZED_MESSAGES = ["report:do_action"];
    ...
    var ReportAction = AbstractAction.extend({
        ...
        start: function () {
            ...
            return Promise.all([this._super.apply(this, arguments), session.is_bound]).then(
                async function () {
        ...
        on_message_received: function (ev) {
            ...
            var message_origin = utils.build_origin(
                ev.originalEvent.source.location.protocol,
                ev.originalEvent.source.location.host
            );
            if (message_origin === this.trusted_origin) {
                ...
                if (
                    !_.isString(message) ||
                    (_.isString(message) && !_.contains(AUTHORIZED_MESSAGES, message))
                ) {
                    return;
                }
