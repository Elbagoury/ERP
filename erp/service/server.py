...
import erp
from erp.modules import get_modules
from erp.modules.registry import Registry
from erp.release import nt_service_name
from erp.tools import config
from erp.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'erp.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    # is included in all Python 3 releases installed on all operating systems supported by ERP.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(erp, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in erp.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="erp.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from erp.addons.base.models.ir_cron import ir_cron
        conn = erp.sql_db.db_connect('postgres')
        with conn.cursor() as cr:
            ...
            while True:
                ...
                registries = erp.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(erp.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="erp.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="erp.service.httpd")
        t.daemon = True
        t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                logger = erp.tests.runner._logger
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('erp: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(erp.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = erp.service.db.list_dbs(True)
        return db_names
    ...
    def start(self):
        ...
        dbconn = erp.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(erp.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            erp.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if erp.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    from erp.tests.common import ERPSuite
            ...
            if config['test_enable']:
                ...
                if post_install_suite.has_http_case():
                    with registry.cursor() as cr:
                        env = erp.api.Environment(cr, erp.SUPERUSER_ID, {})
...
def start(preload=None, stop=False):
    """ Start the erp http server and cron processor.
    """
    global server
    ...
    if erp.evented:
        server = GeventServer(erp.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(erp.http.root)
        ...
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which ERP is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(erp.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not erp.evented:
        if inotify:
