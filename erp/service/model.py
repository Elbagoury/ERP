...
from erp.tools import DotDict
from erp.tools.translate import _, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = erp.registry(db).check_signaling()
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    ...
    result = retrying(partial(erp.api.call_kw, recs, method, args, kw), env)
...
def execute(db, uid, obj, method, *args, **kw):
    with erp.registry(db).cursor() as cr:
...
def retrying(func, env):
    ...
    :param erp.api.Environment env: The environment where the registry
