...
__author__ = erp.release.author
__version__ = erp.release.version

# Also use the `erp` logger for the main script.
...
def report_configuration():
    ...
    """
    config = erp.tools.config
    _logger.info("ERP version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', erp.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = erp.tools.config
    if not erp.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = erp.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            erp.tools.trans_export(config["language"],
...
def import_translation():
    ...
    registry = erp.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = erp.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    erp.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                erp.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except erp.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        erp.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = erp.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the erp server (default command)"""
    def run(self, args):
        main(args)
