...
class Shell(Command):
    """Start erp in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        erp.cli.server.report_configuration()
        erp.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            registry = erp.registry(dbname)
            with registry.cursor() as cr:
                uid = erp.SUPERUSER_ID
                ctx = erp.api.Environment(cr, uid, {})['res.users'].context_get()
                env = erp.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
