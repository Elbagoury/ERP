...
from .. import tools
from .common import TagsSelector, ERPSuite
from .runner import ERPTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'erp.addons.{module}'))

    upgrade_spec = importlib.util.find_spec(f'erp.upgrade.{module}')
    if upgrade_spec:
...
def make_suite(module_names, position='at_install'):
    ...
    return ERPSuite(sorted(tests, key=lambda t: t.test_sequence))
