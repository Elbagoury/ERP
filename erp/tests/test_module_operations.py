...
import erp
from erp.tools import config, topological_sort, unique
from erp.netsvc import init_logger
from erp.tests import standalone_tests
import erp.tests.loader

_logger = logging.getLogger('erp.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with erp.registry(db_name).cursor() as cr:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of ERP modules")
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra ERP modules")
...
def test_full(args):
    ...
    with erp.registry(args.database).cursor() as cr:
        env = erp.api.Environment(cr, erp.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = erp.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        erp.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        erp.tools.config['addons_path'] = ','.join([args.addons_path, erp.tools.config['addons_path']])
        if args.data_dir:
            erp.tools.config['data_dir'] = args.data_dir
        erp.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('erp.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('erp.sql_db').setLevel(logging.CRITICAL)
